# Introdução à Programação com Dados Geoespaciais

Galeria de Jupyter Notebooks do curso:
- [Jupyter Notebooks](./introducao-jupyter-notebook.ipynb)
  
- [Introdução à Programação com a Linguagem Python](./introducao-jupyter-notebook.ipynb) 

- [Tipos Geométricos em Python](./tipos-geometricos.ipynb)

- [Sistemas de Referência de Coordenadas (CRS)](./sistemas-referencia-coordenadas.ipynb)
 
- [Manipulação de Dados Vetoriais - Leitura de Arquivos ESRI Shapefile com a biblioteca Fiona](dados-vetoriais-fiona.ipynb)
